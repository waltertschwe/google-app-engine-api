...
foreach($instances as $instanceName) {
	try {
		...
		if(!empty($getResponse)) {
			...
			if($status == "RUNNING") {
				echo "Instance is running... stopping<br/>";
				$stopResponse = $computeService->instances->stop($project, $zone, $instanceName);
				error_log(json_encode($stopResponse));
			} else {
				echo "<b>" . $instanceName. " </b>is not running current status is = <b>" . $getResponse->status . "</b><br/>";	
			}
		
		} else {
			echo "<b>" . $instanceName . " isb not found or is already stopped.";
		}
	} catch (Exception $e) {
